...
from scikms.config import Config
from scikms.utils.dispatch import Signal
from .consts import USER_PLUGINS_DIR

if TYPE_CHECKING:
    from scikms.app import App
...
class Plugin:
    ...
    def init_config(self, config: Config):
        ...
        names = [self.name]
        # Plugin module names commonly look like scikms_xxx; expose xxx as a
        # shorter alias so user configs can be app.xxx.X=Y rather than
        # app.scikms_xxx.X=Y.
        if self.name.startswith('scikms_'):
            names.append(self.name[len('scikms_'):])
        ...
        try:
            ...
            try:
                entry_points = importlib.metadata.entry_points() \
                    .select(group='scikms.plugins_v1')
            except AttributeError:  # old version does not has `select` method
                entry_points = importlib.metadata.entry_points() \
                    .get('scikms.plugins_v1', [])  # pylint: disable=no-member
        except ImportError:
            import pkg_resources
            entry_points = pkg_resources.iter_entry_points('scikms.plugins_v1')
